...
int main(int argc, char *argv[])
{
  ...
  phy.EnablePcapAll("wifi-stats");
    ...
    double offeredLoad = 150e6 / nWifi;     // 150 Mb/s divided per station
    uint32_t payload = 1472;               // typical payload for performance tests

    OnOffHelper onOff("ns3::UdpSocketFactory", sinkSocket);
    onOff.SetAttribute("OnTime", StringValue("ns3::ConstantRandomVariable[Constant=1]"));
    onOff.SetAttribute("OffTime", StringValue("ns3::ConstantRandomVariable[Constant=0]"));
    onOff.SetAttribute("DataRate", DataRateValue(DataRate(offeredLoad)));
    onOff.SetAttribute("PacketSize", UintegerValue(payload));

    for (uint32_t i = 0; i < nWifi; ++i)
    {
        sourceApplications.Add(onOff.Install(wifiStaNodes.Get(i)));
    }
